...
i() ->
    ...
    [begin
         io:format("~40s ~s : ~p MB ~n", ["Node Total Mnesia Memory", Node, format_memory(rpc:call(Node, mlib_sys, get_total_mnesia_memory, []), mb)])
     end || Node <- Nodes],
    io:format("~n"),
    io:format("****** ----------------------- Memory ----------------------------~n~n"),
    ok.
...
dict(PName) ->
    FileName =  lists:concat(["/tmp/", mlib_tool:to_list(PName),"dictionary", ".txt"]),
            ...
            fun(CaptureData, Acc) ->
                ...
                if Begin =:= 0 andalso Len =:= 0 ->
                    ...
                    true ->
                        ...
                        [M, F, A] = string:tokens(mlib_tool:to_list(R), ":/"),
                        [{mlib_tool:to_atom(M), mlib_tool:to_atom(F), mlib_tool:to_integer(A)}|Acc]
                end
            end, [], Captured),
...
atom_info([("entries: "++Ent)|L], _, Limit) ->
    Entries = mlib_tool:to_integer(Ent),
    atom_info(L, Entries, Limit);
atom_info([("limit: "++Lim)|L], Entries, _) ->
    Limit = mlib_tool:to_integer(Lim),
    atom_info(L, Entries, Limit);
